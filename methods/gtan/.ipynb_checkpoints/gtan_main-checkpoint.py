...
def load_gtan_data(dataset: str, test_size: float):
    prefix = '/scratch/ap8235/pytorch-example/optimized_gnn/data/'
    if dataset == "S-FFSD":
        ...
        g.ndata['label'] = torch.from_numpy(process_labels(labels)).to(torch.long)
        g.ndata['feat'] = torch.from_numpy(process_features(feat_data)).to(torch.float32)
