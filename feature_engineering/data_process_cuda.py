...
@cuda.jit
def calculate_numerical_features_cuda(amounts, times, current_time, time_spans, results):
    idx = cuda.threadIdx.x + cuda.blockIdx.x * cuda.blockDim.x
    if idx < time_spans.size:
        length = time_spans[idx]
        ...
        for i in range(times.size):
            if low_bound <= times[i] <= current_time:
                amt = amounts[i]
                sum_amount += amt
                sum_square += amt * amt
                count += 1
        ...
        if count > 0:
            mean_amount = sum_amount / count
            std_amount = ((sum_square / count) - (mean_amount * mean_amount)) ** 0.5
        else:
            mean_amount = 0.0
            std_amount = 0.0

        # Assigning each result to a specific column in the 2D results array
        results[idx, 0] = mean_amount
        results[idx, 1] = sum_amount
        results[idx, 2] = std_amount
        results[idx, 3] = count
...
def featmap_gen(tmp_df):
    ...
    d_amounts = cuda.to_device(amounts)
    d_times = cuda.to_device(times)
    d_time_spans = cuda.to_device(time_spans)
    results = cuda.device_array((time_spans.size, 4), dtype=np.float64)  # Prepare results array for 4 statistics per time span

    threads_per_block = 1024
    blocks_per_grid = (time_spans.size + threads_per_block - 1) // threads_per_block

    for i, row in tqdm(tmp_df.iterrows(), total=len(tmp_df)):
        current_time = row['Time']
        
        calculate_numerical_features_cuda[blocks_per_grid, threads_per_block](d_amounts, d_times, current_time, d_time_spans, results)
        ...
        feature_dict = {}
        for idx, tname in enumerate(time_names):
            mean_amount = host_results[idx, 0]
            total_amount = host_results[idx, 1]
            std_amount = host_results[idx, 2]
            count = host_results[idx, 3]
            ...
            # Add calculations for bias and any other custom features as needed
